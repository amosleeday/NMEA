...
int DdAmbInfo::TotalUnfixNum(int typeNum)
{
	int cnt=0;
...
DdAmbInfo& DdAmbInfo::Shrink()
{
	int num=pairNum,cnt=0;
	for (int i=0;i<num;i++)
	{
		if (prnList[0][i]!=0)
		{
			prnList[0][cnt]=prnList[1][cnt]=prnList[2][cnt]=prnList[0][i];
			for(int j=0;j<NFREQ;j++)
			{
				fixFlag[j][cnt]=fixFlag[j][i];
				fixSolu[j][cnt]=fixSolu[j][i];
			}
			cnt++;
		}
	}
...
{
	int num=numPhs[0],cnt=0;
	for (int i=0;i<num;i++)
	{
		if (prnlistPhs[0][i]!=0)
		{
			prnlistCod[0][cnt]=prnlistCod[1][cnt]=prnlistCod[2][cnt]=prnlistCod[0][i];
			prnlistPhs[0][cnt]=prnlistPhs[1][cnt]=prnlistPhs[2][cnt]=prnlistPhs[0][i];
			eleRovRov[cnt]=eleRovRov[i];
			eleRovBase[cnt++]=eleRovBase[i];
		}
	}
	for(int i=0;i<3;i++)numCod[i]=numPhs[i]=cnt;
	return *this;
}
