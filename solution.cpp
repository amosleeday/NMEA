...
void PassFixedAmb(DdCtrl ddctrl,DdAmbInfo& curAmb,DdObsInfo obsinfo,math::matrix<double>afix, int resultflag)
{
	...
	{			
		int i,j,k,cnt=0;
		int* prnlistPar=new int[curAmb.TotalUnfixNum()]; //list of unfixed sate
...
void SuperpositionNorEquAmb(DdCtrl ddctrl,math::matrix<double>& PreNorAmb,math::matrix<double> CurNorAmb,DdAmbInfo PreAmbInfo,DdAmbInfo CurAmbInfo)
{
	...
	int* CurPrnlist=new int[CurAmbInfo.TotalUnfixNum()]; //list of unfixed sate
	int* PrePrnlist=new int[PreAmbInfo.TotalUnfixNum()];
...
{
	int dimAmb=ambinfo.TotalUnfixNum();
	...
	if (ambunfixed>0)
	{
		...
		{//partial ambiguities 
			...
			int *posFix=new int[ambinfo.TotalUnfixNum()]; //position to fix
			double*ele=new double[ambinfo.TotalUnfixNum()];
			int *posRest=new int[ambinfo.TotalUnfixNum()];
...
//			if(ambunfixed>0) PassFixedAmb(ambinfo,obsinfo,fixAmb,prnlist,numOfList,resultmode);
		...
		{//partial ambiguities 
			...
			int *posFix=new int[ambinfo.TotalUnfixNum()]; double*ele=new double[ambinfo.TotalUnfixNum()];
			int *posRest=new int[ambinfo.TotalUnfixNum()];
...
/*modify Qaa with S, when the ref sat changed
 *return:   the trans-mat S, which modifies the Qaa from old DD to new DD
 **/
extern math::matrix<double> ModifyQaa(int rowIndex,int colIndex,math::matrix<double> Qaa)
{
	int num=Qaa.RowNo();int rowStart=0,colStart=0;
	math::matrix<double>S(num,num);
	for (int i=0;i<num;i++) S(i,colIndex-1)=-1.0;
	if (rowIndex>=colIndex)
	{
		for (int i=0;i<num;i++)
		{
			...
			{
				rowStart=rowIndex; 
				S(i-1,i)=(rowIndex==colIndex)?-1.0:1.0;
				continue;
			}

			if (i!=rowIndex-1)
				if(i==colIndex-1) colStart=colIndex;
			if (i!=colIndex-1) 
				S(rowStart++,i)=1.0;
		}
	}
